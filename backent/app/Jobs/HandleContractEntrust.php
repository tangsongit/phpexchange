...
class HandleContractEntrust implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($entrust['side'] == 1) {
            ...
            //市价交易 吃单
            //获取可交易列表
            $sellList = ContractEntrust::getContractSellList($entrust['type'], $where_data); //获取合约卖单列表
            if (!blank($sellList)) {
                //有可交易订单 撮单
                DB::beginTransaction();
                try {
                ...
                $flag = false;
                if ($entrust['type'] == 1 || $entrust['type'] == 3) { //如果委托方式为 限价/止盈止损
                    // 获取最新一条成交记录 即实时最新价格
                    $trade_detail = Cache::store('redis')->get('swap:' . 'trade_detail_' . $entrust['symbol']);
                    $realtime_price = $trade_detail['price'];
                    if ($entrust['entrust_price'] >= $realtime_price) $flag = true; //如果委托价格大于市场价格则买入
                } else { //如果为市价交易则买入
                    $flag = true;
                }
                if ($flag) {
                    ...
                    if ($entrust['order_type'] == 1 && $entrust['side'] == 1) { //开仓且买入
                        // 买入开多
                        $service->handleOpenBuyOrder($entrust);
                    } elseif ($entrust['order_type'] == 1 && $entrust['side'] == 2) { //开仓且卖出
                        // 卖出开空
                        $service->handleOpenSellOrder($entrust);
                    } elseif ($entrust['order_type'] == 2 && $entrust['side'] == 1) { //平仓且买入
                        // 买入平空
                        $service->handleFlatBuyOrder($entrust);
                    } elseif ($entrust['order_type'] == 2 && $entrust['side'] == 2) { //平仓且卖出
                        // 卖出平多
        ...
        foreach ($sellList as $sell) {
            ...
            if ($entrust['order_type'] == 1) {
                ...
                if (!blank($buy_user = User::query()->find($entrust['user_id']))) {
                    ...
                    // 更新用户持仓信息
                    $buy_position = ContractPosition::getPosition(['user_id' => $entrust['user_id'], 'contract_id' => $entrust['contract_id'], 'side' => $entrust['side'], 'fencang' => env('IS_COMPARTMENT', 0)]);
                    $buy_position->update([
