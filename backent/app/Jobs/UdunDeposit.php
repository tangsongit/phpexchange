...
class UdunDeposit implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($trade['mainCoinType'] == 0 && $trade['coinType'] == 0) {
            ...
            $note = 'USDT-TRC20';
        } elseif ($trade['mainCoinType'] == 195 && $trade['coinType'] == 'TQRfoUASLX1oiNPbzD72dvRUPMoYuqYgEV') {
            // USDT-TRC20
            $wallet = UserWallet::query()->where(['coin_name' => 'GD', 'wallet_address' => $trade['address']])->first();
            $note = 'USDT-TRC20';
        }
        else {
            return;
