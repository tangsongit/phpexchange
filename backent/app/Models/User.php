...
class User extends Authenticatable implements JWTSubject
{
    ...
    public function update_wallet_and_log(
        ...
        if ($amount < 0 && bccomp($balance, abs($amount)) < 0) {
            ...
            throw new ApiException('UID-' . $this->user_id . ' - ' . $rich_type . ' - ' . $coin_id . ':资产不足');
        }
    ...
    /**
     * @description: 邀请码属于用户，查询邀请码是否存在
     * @param {*} $code
     * @param {*} $user_id
     * @return {*}
     */
    public static function isInviteCodeExist($code, $user_id = null)
    {
        $baseQuery = self::query()->where('invite_code', $code);
        if ($user_id) {
    ...
    /**
     * @description: 获取团队成员列表/无限极下级/链上成员
     * @param {*} $user_id 用户ID
     * @param {*} $users    用户列表
     * @return {*} Array
     */
    public static function getChilds(int $user_id, $users = null)
    {
        if (blank($users)) {
            $users = self::all(['user_id', 'pid', 'is_agency']);
        };
        $childs = [];
    ...
    public static function getParents(int $user_id, $users = null)
    {
        if (empty($users)) {
            $users = self::all();
        }
        ...
        return collect($tree)
            ->reject(function ($v) {
                if ($v->is_agency == 0 && $v->is_place == 0) return 1;
            })
            ->pluck('user_id')
            ->toArray();
    }
