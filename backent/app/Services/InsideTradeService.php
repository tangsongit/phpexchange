...
class InsideTradeService
{
    ...
    public function getHistoryEntrust($user, $params)
    {
        ...
        $data = $buyList->merge($sellList)->sortByDesc('created_at')->each(function ($v) {
            if ($v->status == 2 || $v->status == 3) {
                $trade_money = $v->order_details->sum('trade_money');
                $trade_amount = $v->order_details->sum('trade_amount');
                $v->avg_price = ($trade_amount != 0 && $trade_money != 0) ? number_format($trade_money / $trade_amount, 8, '.', '') : 0;
            }
            if ($v->type == 2 && $v->status == 3) {
                $v->amount  = $v->traded_amount;
            }
        });
        //当前页数 默认1
        $page = request()->page ?: 1;
    ...
    public function handleBuyOrder($tradeBuy)
    {
        ...
        if ($tradeBuy['type'] == 1 || $tradeBuy['type'] == 3) {
            ...
            $surplus_amount = bcMath($surplus_money, $entrust_price, '/', 6);
        }
    ...
    public function deal($buy, $sell, $side = 'buy', $deal_amount = null)
    {
        ...
        if ($side == 'buy') {
            $exchange_amount = $sell['amount'];  //卖单数量
            $unit_price = $sell['entrust_price'];   //卖单委托价格
            $exchange_money = PriceCalculate($unit_price, '*', $exchange_amount, 6); //交易金额（委托价格*委托数量）
            $symbol = $sell['symbol'];  //币对

            $buy_fee = PriceCalculate($exchange_amount, '*', $maker_fee_rate, 6);   //卖单手续费
            $sell_fee = 0;  //卖单手续费
            if ($buy['type'] == 2) { //如果是市价交易
                $freeze_balance = $exchange_money;  //取卖方金额委托数量与价格作为交易金额
            } else {
                $freeze_balance = PriceCalculate($buy['entrust_price'], '*', $exchange_amount, 6); //否则以卖方价格*卖方数量作为交易金额
            }
            ...
            if ($sell['type'] == 2) { //如果是市价交易
                $freeze_balance = $exchange_money;  //取卖方金额委托数量与价格作为交易金额
            } else {
                $freeze_balance = PriceCalculate($sell['entrust_price'], '*', $exchange_amount, 6); //否则以卖方价格*卖方数量作为交易金额
            }
            //买单全部成交
            $buy->update([
        ...
        if ($side == 'buy' && $buy_user) {
            ...
            if ($buy['type'] == 2) { //如果是市价交易,返还多余交易金额（解决交易完成后出现冻结资金的问题）
                $buy_user->update_wallet_and_log($buy['quote_coin_id'], 'freeze_balance', -$buy['money'], UserWallet::asset_account, 'entrust_exchange');
                $buy_user->update_wallet_and_log($buy['quote_coin_id'], 'usable_balance', ($buy['money'] - $freeze_balance), UserWallet::asset_account, 'entrust_exchange');
            } else {    //如果是限价交易
