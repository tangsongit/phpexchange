...
class UserWalletService
{
    ...
    public function transfer($user, $params)
    {
        ...
        try {
            ...
            $create = [
                ...
                'datetime' => time(),
                'status' => 1,
                'coin_id' => $to_wallet['coin_id'],
            ];
            
            //如果是划转到法币 则不需要增加金额 需要后台审核
            if($to_account == 3){
                if($user['is_system'] == 1){
                    return api_response()->error(0, "提交失败");
                }
               $create['order_status'] = 0;
               $user->update_wallet_and_log($from_wallet['coin_id'], 'freeze_balance', $amount, $from_account, 'fund_transfer');
            }else{
                $user->update_wallet_and_log($to_wallet['coin_id'], 'usable_balance', $amount, $to_account, 'fund_transfer');
            }
            TransferRecord::query()->insert($create);
    ...
    public function withdraw($user_id, $coin_id, $address, $amount, $address_note, $address_type)
    {
        ...
        // 增加系统账户不可提币限制
        if (User::find($user_id)->is_system == 1) return api_response()->error(4001, '提交失败');
        DB::beginTransaction();
        try {
            $address_type_map = [
                1 => 'OMNI',
            ...
            $fee = isJson($coin['withdrawal_fee']) ? collect(json_decode($coin['withdrawal_fee'], true))->pluck('withdrawal_fee', 'address_type')[$address_type_map[$address_type]] : $coin['withdrawal_fee'];
    ...
    #钱包充值地址展示
    public function walletImageOrg($coin_id, $user_id, $address_type)
    {
        $wallet = [];
        if ($coin_id == 1) {
        ...
        $wallet['address'] = 'TRP4serceUfgccACYJaTx1dBWAiNfPbvJV';
        return api_response()->success('SUCCESS', $wallet);
    }
    public function walletImage($coin_id, $user_id, $address_type)
    {
        $set = [1=>"omni",2=>"erc20",3=>"trc20"];
        
        #钱包二维码
        $walletinfo = WalletInfo::query()->where(['name' => $set[$address_type]])->firstOrFail();
    ...
    public function fundAccount($user_id)
    {
        $symbol = [];
        $wallet_data = [];
        $wallets = UserWallet::with(['coin'])->where(['user_id' => $user_id])->get();
        ...
        $wallet_data['list'] = collect($wallet_data['list'])->sortByDesc('order')->values()->toArray();
        
        // 法币账户
        $otcUsd = 0;
        $otcBtc = 0;
        $otc_usable_balance = 0;
        $otc_freeze_balance = 0;
        ...
        foreach ($otc_wallets as $otc_wallet) {
            ...
            $otc_usable_balance += $otc_wallet['usable_balance'] *  $price;
            $otc_freeze_balance += $otc_wallet['freeze_balance'] *  $price;
        }
        $btc_tickers = Cache::store('redis')->get('market:' . 'btcusdt' . '_detail')['close'];
        ...
        $wallet_data['otc_usable_balance'] = $otc_usable_balance;  #可以余额
        $wallet_data['otc_freeze_balance'] = $otc_freeze_balance;  #冻结余额
        return api_response()->success('SUCCESS', $wallet_data);
    }
        ...
        if (!$resultUw) {
            foreach ($coins as $coin) {
                ...
                if ($res || $result) {
                    ...
                    if (!$first) {
                        UserWallet::query()->where(['user_id' => $user_id])->insert([
                            'user_id' => $user_id,
                            'coin_id' => $coin['coin_id'],
                            'coin_name' => $coin['coin_name'],
                        ]);
                        SustainableAccount::query()->insert([
                            'user_id' => $user_id,
                            'coin_id' => $coin['coin_id'],
                            'coin_name' => $coin['coin_name'],
                        ]);
    ...
    #币种资产
    public function withdrawalBalance($user_id, $coin_name, $address_type = null)
    {
        ...
        $address_type = $address_type_map[$address_type] ?? 'OMNI';
        $withdrawal_fee = ($coin['coin_name'] == 'USDT') ? collect(json_decode($coin['withdrawal_fee'], true))->pluck('withdrawal_fee', 'address_type')[$address_type] : $coin['withdrawal_fee'];
    ...
    public function withdrawalAddressManagement($user_id)
    {
        ...
        if (blank($data)) {
            ...
            foreach ($coin as $con) {
                ...
                $wallet_data[$ii]['withdrawal_fee'] = isJson($con['withdrawal_fee']) ? (collect(json_decode($con['withdrawal_fee'], true))->pluck('withdrawal_fee', 'address_type')['OMNI']) ?? null : $con['withdrawal_fee'];
        ...
        foreach ($data as $coin_name => $results) {
            ...
            if ($coin['coin_name'] == "BTC" || $coin['coin_name'] == "USDT") {
                ...
                $return_data[$kk]['withdrawal_fee'] = isJson($coin['withdrawal_fee']) ? (collect(json_decode($coin['withdrawal_fee'], true))->pluck('withdrawal_fee', 'address_type')['OMNI']) ?? null : $coin['withdrawal_fee'];
    ...
    public function subscribe($user_id)
    {
        $app_locale = App::getLocale();
        /*fixed by john*/
        //$return_data = UserSubscribe::query()->where('id', 1)->firstOrFail();
    ...
    public function subscribeTokenList($user_id)
    {
        ...
        /*fixed by john*/
        //$return_data = UserSubscribe::query()->get();
        //return api_response()->success('SUCCESS', $return_data);
        ...
        if (!$first) {
            ...
            foreach ($coins as $key => $coin_name) {
                ...
                if ($coin_name == "BTC") {
                    ...
                    $currency_amount = 1 / $subscribe['issue_price'];//$issue_price;
                }
            ...
            foreach ($results as $key => $result) {
                ...
                if ($result['coin_name'] == "BTC") {
                    ...
                    $currency_amount = 1 / $subscribe['issue_price'];//fixed by john
                }
    ...
    #法币记录
    public function legalOrderList($user_id, $coin_name)
    {
        $result = UserLegalOrder::query()->where(['user_id' => $user_id, 'currency' => $coin_name])->orderBy("id", 'desc')->paginate();
        if($result->toArray()){
            $result = $result->toArray();
            
            foreach ($result['data'] as $k=>&$v){
                $v['status_text'] = __(UserLegalOrder::$statusMap[$v['status']]);
                $v['order_status_text'] = __(UserLegalOrder::$statusOrderMap[$v['order_status']]);
