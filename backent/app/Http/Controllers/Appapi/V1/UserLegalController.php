...
";
    ...
    public function buy_sell(Request $request){
        ...
        try {
            ...
            //如果是卖币先冻结用户金额
            if($type == 'sell'){
                $otc_account = \App\Models\OtcAccount::where(['user_id'=>$user['user_id'],'coin_name'=>strtoupper($currency)])->first();
                if($otc_account && $otc_account->usable_balance < $params["number"]){
                    return $this->error('error','余额不足');
                }
            ...
            $url = "https://ct5.biya88.com/" . $type . "/home?sign=" . $sign . '&' . implode("&", $p);
            //创建订单
    ...
    public function order_list(Request $request){
        ...
        return $UserWalletService->legalOrderList($user_id, $coin_name);
