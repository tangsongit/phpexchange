...
class UserWalletController extends ApiController
{
    ...
    public function __construct(UserWalletService $UserWalletService)
    {
        ...
        $this->OtcService = new OtcService;
    }
    ...
    public function wallet_image(Request $request)
    {
        ...
        return $this->UserWalletService->walletImage($coin_id, $user_id, $address_type);
    }
    #钱包划转
    ...
    public function withdrawal_balance(Request $request)
    {
        if ($vr = $this->verifyField($request->all(), [
            ...
            'address_type' =>  'required_if:coin_name,USDT'
        ])) return $vr;
        ...
        $coin_name = $request->input('coin_name');
        $address_type = $request->input('address_type');
        return $this->UserWalletService->withdrawalBalance($user_id, $coin_name, $address_type);
    }
    ...
    public function subscribeToken_list(Request $request)
    {
        ...
        //return $this->UserWalletService->subscribeTokenList_copy($user_id);
    }
    public function getKlineSubscribe(Request $request)
    {
        $datetime = $request->input('datetime');
        $coin = $request->input('coin', 1);
        $compress = $request->input('compress', 0);
        ...
        foreach ($klineData as $k=>$item) {
            
            if($k%8==0)$response['klineData'][] = $item['open'] ;
            
        }
        return $this->successWithData($response);
        //return $compress ? base64_encode(gzcompress(serialize($response))) : $response;
    }
    #立即申购
    ...
    public function recharge_manual_log(Request $request)
    {
        ...
        $user_id = $user['user_id'];
        // 查询充值记录
        $data = DB::table('recharge_manual')
            ->where('uid', $user_id)
            ->whereNull('deleted_at')
            ->select('num', 'status', 'created_at')
            ->orderByDesc('id')
            ->paginate($limit);
     ...
     */
    public function otcWalletLogs(Request $request)
    {
        if ($vr = $this->verifyField($request->all(), [
            'symbol' => '', // 名称 参数格式：
        ])) return $vr;
        ...
        $data = $this->OtcService->otcWalletLogs($user, $params);
        return $this->successWithData($data);
    }
}
