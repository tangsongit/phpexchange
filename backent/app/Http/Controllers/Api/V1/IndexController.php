...
use Illuminate\Support\Facades\DB as Dbb;
...
class IndexController extends ApiController
{
    ...
    public function indexList()
    {
        ...
        foreach ($market as $key => $items) {
            ...
            foreach ($items["marketInfoList"] as  $coin) {
                ...
                $symbols[$k]["price"] = $data["price"];//$coin["close"];//john fixed at 20221001
                $symbols[$k]['increase'] = (float)$data["increase"];//john fixed
                $symbols[$k]['increaseStr'] = $data["increaseStr"];//john fixed
        ...
        $bazaar2 = Article::query()
            ->where(["category_id" => $category_id2, 'status' => 1])
    ...
    public function op()
    {
        ...
        try {
            //读取多条数据
            $data = UserWalletErrorLogs::query()->limit(10)->get()->toArray();
            //print_r($data);

            //分页数据
            $data = UserWalletErrorLogs::query()->paginate(1)->toArray();
            //print_r($data);

            //读取单条
            $data = UserWalletErrorLogs::query()->where(array('id' => 2))->first()->toArray();
            //print_r($data);

            //更新数据
            $is = UserWalletErrorLogs::query()->where(array('id' => 2))->update(
                [
                    'account_type' => 2,
                    'rich_type' => 'money',
                    'error_info' => '哈哈3333'
            ...
            $is = UserWalletErrorLogs::query()->insert(
                [
                    'user_id'   => 8,
                    'account_type' => 1,
                    'rich_type' => 'money',
                    'error_info' => '资产不足' . mt_rand(1000, 9999),
                    'coin_id'   => 10,
                    'coin_name' => 'USDT',
                    'amount'    => -10,
                    'error_amount' => -10,
            ...
            //删除数据
            $is = UserWalletErrorLogs::query()->where(array('id' => 4))->delete();
            if (!$is) {
            ...
            Dbb::rollBack();
            info('=====测试回调失败======', array('error' => $e->getMessage()));
