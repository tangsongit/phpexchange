...
class InsideTradeController extends ApiController
{
    ...
    public function getMarketInfo(Request $request)
    {
        ...
        $sellList = array_reverse(Cache::store('redis')->get('market:' . $symbol . '_depth_sell') ?: []);
        $tradeList = array_reverse(Cache::store('redis')->get('market:' . $symbol . '_newPriceBook') ?: []);
    ...
    public function storeEntrust(Request $request)
    {
        ...
        if ($vr = $this->verifyField($request->all(), [
            ...
            'amount' => 'nullable|required|numeric|min:0', //委托数量
            'total' => 'nullable|numeric|min:0.00000001', //委托总价 市价委托买单
        ])) return $vr;
        $user = $this->current_user();
    ...
    /**
     * @description: 获取币币交易行情
     * @param {*}
     * @return {*}
     */
    public function allticker()
    {
        $res = \App\Workerman\Option\Events::getTickerList();
        $data['ticker'] = collect($res)->map(function ($v) {
            try {
                return  [
                    'symbol' => strtolower("{$v['base_coin_name']}_{$v['quote_coin_name']}"), //交易对
                    'buy'   => $v['bid'], //买一价
                    'high'  => $v['high'], //最高价
                    'last'  => $v['lastPrice'], //最新成交价格
                    'low'   => $v['low'],  //最低价格
                    'sell'  => $v['ask'],  //卖一价
    ...
    {
        $res = \App\Workerman\Option\Events::getTickerList();
        $data = collect($res)->map(function ($v) {
            return  [
                'symbol' => $v['base_coin_name'], //币种
                'anchor' => $v['quote_coin_name'], //交易区
                'last_price'  => $v['close'],
                'volumn_24h'  => $v['vol'],
                'amount_24h'  => $v['amount'],
                'price_update_at' => time()
            ];
        });
        return json_encode($data);
    }
