...
class MarketController extends ApiController
{
    ...
    {
         return $this->successWithData(['price_cny' => 6.47]);
        try {
            ...
            for ($i = 0; $i < 3; $i++) {
                ...
                if (!blank($detail)) break;
            }
            // if (blank($detail)) return $this->error();
            if (blank($detail)) return $this->successWithData(['price_cny' => 6.47]);
