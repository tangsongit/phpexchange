...
use App\Models\DataBt;
use Illuminate\Support\Carbon;
use Illuminate\Console\Command;

class FakeKline1 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'fakeKline1';
    ...
    {
        // 最后一条1dayK线
        $last = DataBt::query()->where('is_day', 1)->orderByDesc('Date')->first();
        if (blank($last)) {
            //Y-m-d H：i：s  开始结束时间
            ...
            $decimal = 100000;
            $min_rate = get_setting_value('min_rate', 'coin1', 0.04);
            $max_rate = get_setting_value('max_rate', 'coin1', 0.08);
            if ($min_rate < 0) $min_rate = 0;
            if ($max_rate >= 1) $max_rate = 0.99;
            $up_or_down = get_setting_value('up_or_down', 'coin1', 1);
    ...
    private function fake_1min_kline($change_rate, $base_price, $target_price, $start_date, $end_date)
    {
        ...
        $is_sharp = get_setting_value('is_sharp', 'coin1', 0);
        if ($is_sharp == 1) {
        ...
        while ($current < $end) {
            ...
            // 获取上一条
            $prev = DataBt::query()->where($flag, 1)->where('Date', $current - $seconds)->first(); // 上一条K线
            if (blank($prev)) {
                $prev = DataBt::query()->where('is_day', 1)->orderByDesc('Date')->first();
            }
    ...
    public function fake_period_kline($period, $start_date, $end_date)
    {
        ...
        if ($period == '1week') {
            ...
            $open = DataBt::query()->where('is_1min', 1)->where('Date', '>=', $where_start)->orderBy('Date', 'asc')->first()['Open'] ?? null;
            $close = DataBt::query()->where('is_1min', 1)->where('Date', '<=', $where_end)->orderBy('Date', 'desc')->first()['Close'] ?? null;
            $high = DataBt::query()->where('is_1min', 1)->whereBetween('Date', [$where_start, $where_end])->max('High');
            $low = DataBt::query()->where('is_1min', 1)->whereBetween('Date', [$where_start, $where_end])->min('Low');
            $volume = mt_rand(535012, 2009870) . '.' . mt_rand(10524, 99870);
            $amount = mt_rand(535012, 2009870) . '.' . mt_rand(10524, 99870);

            DataBt::query()->updateOrCreate(
                [
                    'Date' => $where_start,
                    $flag => 1,
                ],
            ...
            while ($current < $end) {
                ...
                $open = DataBt::query()->where('is_1min', 1)->where('Date', $where_start)->value('Open');
                $close = DataBt::query()->where('is_1min', 1)->where('Date', $where_end)->value('Close');
                $high = DataBt::query()->where('is_1min', 1)->whereBetween('Date', [$where_start, $where_end])->max('High');
                $low = DataBt::query()->where('is_1min', 1)->whereBetween('Date', [$where_start, $where_end])->min('Low');
