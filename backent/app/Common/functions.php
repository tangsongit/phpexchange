...
function SMSTemplates()
{
    ...
    $map = [
        ...
        'cn' => '【BitWell】Dear user, your SMS verification code is [%s]。',
        'en' => '【BitWell】Dear user, your SMS verification code is [%s]。',
        'tw' => '【BitWell】親愛的用戶，您的短信驗證碼是[%s]，3分鐘內有效，非自己操作請忽略',
        'tr' => '【BitWell】Sevgili kullanıcı, SMS doğrulama kodunuz [%s], 3 dakika içinde geçerlidir, kendiniz çalıştırılırsa lütfen göz ardı edin',
        'jp' => '【BitWell】親愛なるユーザsmsにんしょー認証コードは[%s]で、3ふん分いない以内にゆーこー有効です',  // 日本
        'kor' => '【BitWell】친애하는 사용자, 당신의 SMS 검증 코드는 [%s] 이며, 3분 이내에 유효합니다, 당신이 작동하지 않는 경우 무시하십시오', // 韩语
        'de' => '【BitWell】Sehr geehrter Benutzer, Ihr SMS-Verifizierungscode ist [%s], gültig innerhalb von 3 Minuten, bitte ignorieren, wenn es nicht von Ihnen selbst betrieben wird',   // 德国
        'it' => '【BitWell】Caro utente, il tuo codice di verifica SMS è [%s], valido entro 3 minuti, ignorare se non è gestito da te stesso',   // 意大利
        'nl' => '【BitWell】Hyvä käyttäjä, tekstiviestin vahvistuskoodisi on [%s],voimassa 3 minuutin kuluessa, ohita, jos sitä ei käytä itse',   // 芬兰
        'pl' => '【BitWell】Drogi użytkowniku, Twój kod weryfikacyjny SMS jest [%s], ważny w ciągu 3 minut, zignoruj, jeśli nie jest obsługiwany przez ciebie',   // 波兰
        'pt' => '【BitWell】Caro usuário, seu código de verificação de SMS é [%s], válido dentro de 3 minutos, por favor ignore se ele não for operado por você mesmo',   // 葡萄牙
        'spa' => '【BitWell】Estimado usuario, su código de verificación SMS es [%s], válido dentro de 3 minutos, por favor ignore si no es operado por usted mismo',  // 西班牙
        'swe' => '【BitWell】Kära användare, din SMS-verifieringskod är [%s], giltig inom 3 minuter, vänligen ignorera om den inte drivs av dig själv',  // 瑞典
        'uk' => '【BitWell】Шановний користуваче, ваш SMS-код підтвердження [%s], дійсний протягом 3 хвилин, будь ласка, проігноруйте, якщо він не працює самостійно'   // 乌克兰
    ];
...
function sendEmailCode($email, $scene = 'verify_code')
{
    ...
    Mail::send('emails.verify_code', ['code' => $code], function ($message) use (&$email) {
        $message->to($email, 'TXSmall')->subject('BitWell');
    }); //dd(Mail::failures());
    if (Mail::failures()) {
        return '发送失败';
    } else {
...
/*发送邮箱划转提醒*/
function sendEmailTransfer($email, $type= 1)
{
    Mail::send($type,['code' => 111],function ($message) use($email){
        $message->to($email)->subject('Transfer remind'); 
...
/*发送短信划转提醒*/
function sendPhoneTransfer($phone, $content){
    $chuanglanSms = new ChuangLanSms;

    $result = $chuanglanSms->sendSMS($phone, $content);
    return true;
}
