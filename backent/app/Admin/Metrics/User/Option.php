...
class Option extends Bar
{
    ...
    public function handle(Request $request)
    {
        ...
        $builder1 =
            ...
            $builder12 =
            $builder13 =
            OptionSceneOrder::query()->whereHas('user', function ($q) use ($user_id) {
                $q->where('user_id', $user_id);
            })->where(['bet_coin_id' => $coin_id]);
        ...
        // 卡片内容
        $this->withContent($user_id, $value1, $value2, $value3, $value4, $value5, $value6);
     ...
     * @return $this
     */
    public function withContent($user_id, string $value1, string $value2, string $value3, string $value4, string $value5, string $value6)
    {
        // 根据选项显示
        $label = strtolower(
...
<div class="d-flex p-1 flex-column justify-content-between" style="padding-top: 0;width: 100%;height: 100%;min-height: {$minHeight}">
    ...
    <a href="{$admin_url}/option-order?user_id={$user_id}" class="btn btn-primary shadow waves-effect waves-light">View Details <i class="feather icon-chevrons-right"></i></a>
</div>
HTML
        );
    }
