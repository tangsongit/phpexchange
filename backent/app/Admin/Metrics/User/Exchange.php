...
class Exchange extends Bar
{
    ...
    public function handle(Request $request)
    {
        ...
        $builder1 = InsideTradeOrder::query()->where(function ($q) use ($user_id) {
            $q->whereHas('buy_user', function ($q) use ($user_id) {
                $q->where('user_id', $user_id);
            })
                ->orWhereHas('sell_user', function ($q) use ($user_id) {
                    $q->where('user_id', $user_id);
                });
        })->where('base_coin_id', $coin_id);
        $builder2 = InsideTradeOrder::query()->where(function ($q) use ($user_id) {
            $q->whereHas('buy_user', function ($q) use ($user_id) {
                $q->where('user_id', $user_id);
            })
                ->orWhereHas('sell_user', function ($q) use ($user_id) {
        ...
        // 卡片内容
        $this->withContent($user_id, $total_vol, $total_fee, $today_vol, $today_fee, '+5.2%');
     ...
     * @return $this
     */
    public function withContent($user_id, $total_vol, $total_fee, $today_vol, $today_fee, $style = 'success')
    {
        // 根据选项显示
        $label = strtolower(
...
<div class="d-flex p-1 flex-column justify-content-between" style="padding-top: 0;width: 100%;height: 100%;min-height: {$minHeight}">
    ...
    <a href="{$admin_url}/inside-trade-order?user_id={$user_id}" class="btn btn-primary shadow waves-effect waves-light">View Details <i class="feather icon-chevrons-right"></i></a>
</div>
HTML
        );
    }
