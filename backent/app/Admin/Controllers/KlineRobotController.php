...
class KlineRobotController extends AdminController
{
    ...
    public function kline_save(Request $request)
    {
        ...
        $this->symbol = $symbol;
        $klineList = collect($post['klineFormData'])->map(function ($v) {
            return json_encode($v);
        })->toArray();
        // 获取历史数据(使用历史数据对比差异，只更改差异的部分数据)
        $klineData = $model::query()
            ->where('datetime', '>=', $config['datetime'])
            ->where('datetime', '<', Carbon::parse($config['datetime'])->addDay()->toDateTimeString())
            ->where('is_30min', 1)
            ->select(['Date as timestamp', 'datetime', 'Open as open', 'Close as close', 'High as high', 'Low as low', 'Volume as volume', 'Amount as amount'])
            ->get()
            ->map(function ($v) {
        ...
        $klineList = collect(array_diff_uassoc($klineList, $klineData, function ($v1, $v2) {
            if ($v1 == $v2) {
                return 0;
            } else {
                return -1;
        ...
        try {
            ...
            // 获取当天K线最后一条数据时间
            $last_data_time = $model::query()->whereDate('datetime', $config['datetime'])->latest('datetime')->value('Date');
            // 保存K线
            foreach ($klineList as $kline) {
                if ($kline['timestamp'] < time() && $kline['timestamp'] < $last_data_time) {
                    continue;
                }
