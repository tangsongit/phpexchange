...
class CreateInsideTradeSellTable extends Migration
{
	...
	public function up()
	{
		Schema::create('inside_trade_sell', function (Blueprint $table) {
			...
			$table->boolean('entrust_type')->default(2);
			...
			$table->integer('base_coin_id')->unsigned()->default(0)->index('exchange_coin_id')->comment('卖出的币种');
			$table->decimal('amount', 20, 8)->unsigned()->default(0.00000000)->comment('委托数量');
			$table->decimal('traded_amount', 20, 8)->unsigned()->default(0.00000000)->comment('已成交数量');
			$table->decimal('money', 20, 8)->unsigned()->nullable()->default(0.00000000);
			$table->decimal('traded_money', 20, 8)->unsigned()->nullable()->default(0.00000000)->comment('实际成交交易额');
