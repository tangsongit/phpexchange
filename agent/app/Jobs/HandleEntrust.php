...
class HandleEntrust implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($entrust['entrust_type'] == 1){
            ...
            //市价交易 吃单
            //获取可交易列表
            $sellList = InsideListSell::getSellTradeList($entrust['type'],$where_data);
            if(blank($sellList)){
                //卖单盘口 没有可交易订单 则先挂单
                $order_data = [
                    'user_id' => $entrust['user_id'],
                ...
                InsideListBuy::query()->create($order_data);
            }else{
                //有可交易订单 撮单
                DB::beginTransaction();
                try{
            ...
            //市价交易 吃单
            //获取可交易列表
            $buyList = InsideListBuy::getBuyTradeList($entrust['type'],$where_data);
            if(blank($buyList)){
                //卖单盘口 没有可交易订单 则先挂单
                $order_data = [
                    'user_id' => $entrust['user_id'],
                ...
                InsideListSell::query()->create($order_data);
            }else{
                //有可交易订单 撮单
                DB::beginTransaction();
                try{
    ...
    public function handleBuyTrade($entrust,$sellList)
    {
        ...
        foreach ($sellList as $sell){
            if( $tradeSell = InsideTradeSell::query()->where('order_no',$sell['order_no'])->first() ){
                //获取可交易量、可交易额
                if($entrust['type'] == 1){
        ...
        if($entrust['type'] == 1){
            if($entrust_amount > 0){
                ...
                InsideListBuy::query()->create($list_buy_data);
            }
        }else{
            if($entrust_money > 0){
                $list_buy_data = [
    ...
    public function handleSellTrade($entrust,$buyList)
    {
        ...
        foreach ($buyList as $buy){
            if( $tradeBuy = InsideTradeBuy::query()->where('order_no',$buy['order_no'])->first() ){
        ...
        if($entrust_amount > 0){
            if($entrust['type'] == 1){
                ...
                InsideListSell::query()->create($list_sell_data);
            }else{
                $list_sell_data = [
                    'user_id' => $entrust['user_id'],
                    'order_no' => $entrust['order_no'],
