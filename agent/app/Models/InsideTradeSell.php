...
class InsideTradeSell extends Model
{
    ...
    public static function getSellTradeList($type, array $where_data)
    {
        $where = [
            ...
            ['status', 'in', [InsideTradeSell::status_wait, InsideTradeSell::status_trading]],
        ];
        if ($type == 1 || $type == 3) {
            //限价交易 市价挂单优先 然后取符合条件的按价格排序
            $market_entrust = DB::table('inside_trade_sell')->where($where)->where('type', 2)->orderBy('created_at');
