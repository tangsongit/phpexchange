...
class InsideTradeBuy extends Model
{
    ...
    public static function getBuyTradeList($type,array $where_data)
    {
        $where = [
            ...
            ['status','in',[InsideTradeBuy::status_wait,InsideTradeBuy::status_trading]],
        ];
        if($type == 1 || $type == 3){
            //限价交易 市价挂单优先 然后取符合条件的按价格排序
            $market_entrust = DB::table('inside_trade_buy')->where($where)->where('type',2)->orderBy('created_at');
