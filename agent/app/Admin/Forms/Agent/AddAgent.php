 ...
 * @LastEditors: GuaPi
 * @LastEditTime: 2021-08-12 15:53:48
 */
...
class AddAgent extends Form
{
    ...
    public function handle(array $input)
    {
        ...
        if (blank(
            User::query() //判断该代理是否有权升级该用户
                ->where('user_id', $input['user_id'])
                ->where('is_agency', 0)
                ->where('pid', $this->agent_id)
                ->first()
        ...
        $validator = Validator::make($input, [
            ...
            "rebate_rate" => "numeric|min:0|max:" . $min_rate,
    ...
    public function form()
    {
        ...
        $this->text('user_id', '代理商UID')->required();
        $this->rate('rebate_rate', '手续费分佣率(默认)')->required()->help('默认手续费分佣率，（币币、申购、合约、期权）为空时将会默认采用此分佣率进行计算,你的默认分佣比例为' . $agent_rate['rebate_rate'] . '%,请输入小于该值');
        $this->radio('详细设置')
