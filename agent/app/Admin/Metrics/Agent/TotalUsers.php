...
class TotalUsers extends Round
{
    ...
    public function handle(Request $request)
    {
        ...
        $total = User::query()->whereIn('user_id', $base_ids)->count();
        $total_auth = User::query()->whereIn('user_id', $base_ids)->where('user_auth_level', User::user_auth_level_top)->count();
        $today = User::query()->whereIn('user_id', $base_ids)->whereDate('created_at', $today_date)->count();
        $today_auth = User::query()->whereIn('user_id', $base_ids)->whereDate('created_at', $today_date)->where('user_auth_level', User::user_auth_level_top)->count();
...
<div class="d-flex p-1 flex-column justify-content-between" style="padding-top: 0;width: 100%;height: 100%;min-height: {$minHeight}">
    ...
    <a href="{$admin_url}/user/team-list?agent_id={$referrer}" class="btn btn-primary shadow waves-effect waves-light">View Details <i class="feather icon-chevrons-right"></i></a>
</div>
HTML
        );
    }
