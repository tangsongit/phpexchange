...
class SubscribeSettlementController extends AdminController
{
    ...
    public function statistics()
    {
        ...
        // 申购
        $subscribe = UserSubscribeRecord::query()->whereHas('user', function ($q) use ($baseAgentIds) {
        ...
        $con = '<code>申购金额：' . (float)$usdt_amount . 'USDT</code> ';
    ...
    {
        $baseAgentIds = Agent::getBaseAgentIds(Admin::user()->id);
        $builder = User::query()->where('is_system', 0)->where('is_agency', 0)->whereIn('referrer', $baseAgentIds);
